...
-(id) init {
    self = [super initWithImageNamed: @"tank1"];
    self.x = 0;
    self.y = 0;
    self.width = 36;
    self.height = 36;
...
-(void) update {
    ...
    if (self.state != 5) {
        ...
        if (myFrame == 0) {
            [self setTexture: [SKTexture textureWithImageNamed: @"tank1"]];
        }
        else if (myFrame == 1) {
            [self setTexture: [SKTexture textureWithImageNamed: @"tank2"]];
            
        }
        else if (myFrame == 2) {
            [self setTexture: [SKTexture textureWithImageNamed: @"tank3"]];
...
-(BOOL) move : (float) vx : (float) vy {
    ...
    if (SW.walkable == YES && SE.walkable == YES && NW.walkable == YES && NE.walkable == YES) {
        ...
        if ((SWTank != NO && SWTank != self) || (SETank != NO && SETank != self)||(NWTank != NO && NWTank != self) ||(NETank != NO && NETank != self)) {
            self.state = 1;
            return NO;
        }
        else {
